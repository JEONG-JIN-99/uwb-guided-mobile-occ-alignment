...
def run_packet_experiment(node_id="rx"):
    parser = argparse.ArgumentParser(
        description=(
            f"{node_id.upper()} quick test: align immediately on each latest "
            "UWB packet without Chrony or a shared UTC start."
        )
    )
    ...
    try:
        ...
        print(
            f"[READY] {node_id.upper()} packet quick test, "
        ...
        while sample_index < args.samples:
            ...
            try:
                ...
                nominal_sec = sample_index * args.nominal_period_sec
                actual_sec = (
