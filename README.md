...
* **`gimbal/gimbal_controller_yaw.py`**
  * 수신된 GPS 위치 데이터를 기반으로 짐벌의 Yaw 각도를 계산하고 PCA9685 서보 드라이버로 모터를 제어합니다.
...
UWB로 정렬하고, 지정한 안정화 시간 이후 QR 인식률을 측정합니다.
...
결과는 기본적으로 `result/dynamic_qr/` 아래 CSV 파일과 QR 인식 실패 프레임으로
저장됩니다.
...
- 실험 종료 후 짐벌을 0도 상대 방향으로 복귀시키고 PCA9685 제어 신호와 카메라 자원을 정리합니다.
...
- GPS/UWB 입력에 따라 ServoKit 각도가 1.8도씩 증가 또는 감소하는지 확인합니다.
