...
5. 스마트폰 IP Webcam 영상에서 QR 코드를 인식합니다.
   ...
   - 인식 프레임은 `result/socket_server_test/detect_frame/`에 저장됩니다.
6. 결과는 `result/<실험 코드>/` 아래 CSV 파일로 저장됩니다.
   - `gps_t_result.csv`, `gps_a_result.csv`
   - `uwb_t_result.csv`, `uwb_a_result.csv`

### 정적 alignment 실험

`docs/static_alignment.md`의 절차는 `code/experiment/static_alignment/`에 구현되어 있습니다. 실행 전에
`code/experiment/static_alignment/config.yaml`의 RealSense 장치 번호를 실제 환경에 맞게 변경하세요. QR 내용은 비교하지 않고 QR 검출 여부만 판정합니다.

```bash
python code/experiment/static_alignment/main.py camera-test
python code/experiment/static_alignment/main.py show-plan --distance 2 --tx-azimuth=-60
python code/experiment/static_alignment/main.py run --distance 2 --tx-azimuth=-60
```

중단된 블록은 첫 실행 시 표시된 디렉토리를 지정하여 재개합니다.

```bash
python code/experiment/static_alignment/main.py run --distance 2 --tx-azimuth=-60 \
  --run-dir result/static_alignment/run_YYYYMMDD_HHMMSS_d2_tx-60
```
